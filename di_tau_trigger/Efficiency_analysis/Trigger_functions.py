...
def is_mRNN(event,tau,branch='TrigMatched_rnn_HLTptfl',m0=0.35,m1=0.03,mm=0.105):
    ''' Checks if a tau in an event passes the medium RNN cut
        ...
        :return: bool
    '''    
    if branch == 'TrigMatched_rnn_HLTptfl':

        if df['TrigMatched_prong_HLTptfl'][event][tau]==0:
            return df[branch][event][tau]>m0
        if df['TrigMatched_prong_HLTptfl'][event][tau]==1:
            return df[branch][event][tau]>m1
        if df['TrigMatched_prong_HLTptfl'][event][tau]>=2:
    ...
    if branch == 'TrigMatched_rnn_HLTetafl':
        
        if df['TrigMatched_prong_HLTetafl'][event][tau]==0:
            return df[branch][event][tau]>m0
        if df['TrigMatched_prong_HLTetafl'][event][tau]==1:
            return df[branch][event][tau]>m1
        if df['TrigMatched_prong_HLTetafl'][event][tau]>=2:
            return df[branch][event][tau]>mm
...
# def pt_cond2(event,index,i,pt0,pt1): #"index" is index of the tau of concern, i is the index of the tau pair
#     ''' This returns True or False for a particular hlteta tau if it can be paired up 
#     with the second tau and pass the pt cut'''
#     # Check if both the taus are above pt2 and check if atleast one of them is above pt2
#     condition1 = df['TrigMatched_Taus_HLTetafl'][event][index].Pt()>pt1
#     condition1 &= df['TrigMatched_Taus_HLTetafl'][event][i].Pt()>pt1
#     condition1 &= (df['TrigMatched_Taus_HLTetafl'][event][index].Pt()>pt0 or df['TrigMatched_Taus_HLTetafl'][event][i].Pt()>pt0)
...
def DR_L1Topo_cond(event,index,i,min_DR = 0.3, max_DR = 3):
    ''' Checks for a particular tau if it can be paired up with the second tau and pass the angular separation cut for the L1Topo trigger
        ...
        :i: int (index of the pair tau)
        :min_DR: float (Minimum angular separation condition)
        :max_DR: float (Maximum angular separation condition)
        :return: bool
    '''
    hltptDR_cond = df['TrigMatched_Taus_HLTptfl'][event][index].DeltaR(df['TrigMatched_Taus_HLTptfl'][event][i]) > min_DR 
    hltptDR_cond &= df['TrigMatched_Taus_HLTptfl'][event][index].DeltaR(df['TrigMatched_Taus_HLTptfl'][event][i]) < max_DR
    if hltptDR_cond:
        return True
    else:
        return False
...
def HLT_L1Topo_cond(event,tau_i,pt0=30,pt1=20, no_RNN = 440, m_RNN =280,m0=0.35,m1=0.03,mm=0.105,l0=0.1,l1=0.01,lm=0.06, min_DR =0.3,max_DR =3):
    '''Checks if the L1Topo trigger condition is satisfied for a specific tau in an event 
        ...
        :pt0: float (leading tau pT threshold)
        :pt1: float (pair tau pT threshold)
        :no_RNN: pT threshold above which there is no RNN ID requirement
        :mRNN: pT threshold below which there is medium RNN requirement
        :m0: float (0 prong medium RNN working point)
        :m1: float (1 prong medium RNN working point)
        :mm: float (multi prong medium RNN working point)
        :l0: float (0 prong loose RNN working point)
        :l1: float (1 prong loose RNN working point)
        :lm: float (multi prong loose RNN working point)
        :min_DR: float (Minimum angular separation condition)
        :max_DR: float (Maximum angular separation condition)
        :return: bool
    '''
    if len(df['TrigMatched_Taus_HLTptfl'][event])>=2:
        ptflag = 0
        RNN_flag = 0
...
def DR_4J12_cond(event,index,i,min_DR = 0.3):
    ''' Checks for a particular tau if it can be paired up with the second tau and pass the angular separation cut for the 4J12 trigger
        ...
        :return: bool
    '''
    hltptDR_cond = df['TrigMatched_Taus_HLTetafl'][event][index].DeltaR(df['TrigMatched_Taus_HLTetafl'][event][i]) > min_DR
    if hltptDR_cond:
        return True
    else:
        return False
...
def HLT_4J12_cond(event,tau_i,pt0=30,pt1=20, no_RNN = 440, m_RNN =280,m0=0.35,m1=0.03,mm=0.105,l0=0.1,l1=0.01,lm=0.06, min_DR =0.3):
    '''Checks if the 4J12 trigger condition is satisfied for a specific tau in an event 
        ...
        :return: bool
    '''    
    if len(df['TrigMatched_Taus_HLTetafl'][event])>=2:
        ptflag = 0
        RNN_flag = 0
        #Checking if tau_i has a pair that satidsfies the hltpt condition by looping over all taus 
