...
setup_baseprojects() {
    ...
    for project in ${UserAnalysis_BASEPROJECTS}; do
        ...
        fname=${UserAnalysis_DIR}/../../../../${project}/${UserAnalysis_VERSION}/InstallArea/${UserAnalysis_PLATFORM}/setup.sh
        if [ -f ${fname} ]; then
            . ${fname} mode
        fi
        unset fname
...
if [ "x${ENV_VAR_VALUE}" = "x" ]; then
    ...
    export UserAnalysis_PLATFORM=x86_64-centos7-gcc11-opt
    export UserAnalysis_VERSION=0.0.0

    # Base projects used for the build of UserAnalysis:
    UserAnalysis_BASEPROJECTS="Athena"
